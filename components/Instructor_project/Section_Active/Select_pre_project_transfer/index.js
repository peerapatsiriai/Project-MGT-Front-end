...
export default function Select_pre_project_transfer({ activeIndex, setActiveIndex, sectionIdData }) {
  // data variables
  ...
  const handleRowSelect = (preprojectId) => {
    // ตรวจสอบว่า ตัวแปร preprojectId มีอยู่ใน selectedRows หรือไม่
    const isSelected = selectedRows.includes(preprojectId);

    // ถ้ามีข้อมูลอยู่ใน selectedRows ให้นำออก
    if (isSelected) {
      setSelectedRows((prevSelectedRows) => prevSelectedRows.filter((id) => id !== preprojectId));
    } else {
      // ถ้าไม่มีข้อมูลอยู่ใน selectedRows ให้เพิ่มเข้าไป
      setSelectedRows((prevSelectedRows) => [...prevSelectedRows, preprojectId]);
    }
  };
  ...
  //=====================================End Select row Functions================================//

  //=====================================Insert data Functions================================//
  const handleInsertSubmit = (e) => {
    e.preventDefault();

    if (!selectedRows || !sectionIdData) {
      Swal.fire({
        icon: 'error',
    ...
    const data = {
      preproject_list: selectedRows,
      section_id: sectionIdData,
    };

    // console.log('Insert Select data', data);
      ...
      <div className='wrapper-content'>
        <div style={{ width: '100%' }}>
          ...
          {/* <div className='action__body w-full mb-40'>
            ...
            <h2>Project Selection Trans page</h2>
          ...
          <div className='heading-section'>
            <h2 className='tf-title pb-30'>PLEASE SELECT PROJECT TO TRANSFER...</h2>
          </div>
                  ...
                  projectdata.map((project) => (
                    ...
                    >
                      ...
                      <div className='column'>
                        <input
                          type='checkbox'
                          checked={selectedRows.includes(project.preproject_id)}
                          onChange={() => handleRowSelect(project.preproject_id)}
                        />
                      </div>
                      {/* End Checkbox Column */}
