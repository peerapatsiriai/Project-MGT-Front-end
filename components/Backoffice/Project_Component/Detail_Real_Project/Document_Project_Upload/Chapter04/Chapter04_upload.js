...
const Chapter04_upload = ({ activeIndex, setActiveIndex, crossdataProject }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API}api/project-mgt/project?project_id=${projectID}`);

        setDocumentName('Chapter04_' + response.data.PreprojectData[0].preproject_name_th);
      } catch (error) {
        console.error(error);
      }
    };
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API}api/project-mgt/getallonedocumenttypeproject?project_id=306&document_type=บทที่4`
        );
  ...
  // ฟังก์ชันสำหรับ ส่งเอกสาร Chapter04
  const handleChapter04Upload = async () => {
    const docType = 'Chapter04';
    try {
      // ประกอบร่างชื่อใหม่
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API}api/project-mgt/getallonedocumenttypeproject?project_id=${projectID}&document_type=บทที่4`
        );
  ...
  const handlePreview = async (FileName) => {
    try {
      const previewResponse = await fetch('/api/download', {
        ...
        body: JSON.stringify({ fileName: FileName, docType: 'Chapter04' }),
        responseType: 'blob',
      });
    ...
    <div>
      <Box style={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
        <Card style={{ width: '60%', borderRadius: 15 }}>
          ...
          >
            <PostAddIcon style={{ marginRight: '0.2rem', height: '5vh' }} /> อัปโหลดเอกสาร Chapter 4
          </Typography>
            ...
            >
              ...
              <Grid justifyContent='center'>
                <WhiteBlackButton
                  ...
                  onClick={() => {
                    handleChapter04Upload();
                    setRefreshFlag((prevFlag) => !prevFlag); // เรียกใช้ useEffect ใน Chapter04Record
                  }}
                >
                  ส่ง
                </WhiteBlackButton>
...
export default Chapter04_upload;
