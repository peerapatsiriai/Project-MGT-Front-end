...
const Chapter02_upload = ({ activeIndex, setActiveIndex, crossdataProject }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setDocumentName('Chapter02_' + response.data.PreprojectData[0].preproject_name_th);
      } catch (error) {
        console.error(error);
      }
    };
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API}api/project-mgt/getallonedocumenttypeproject?project_id=306&document_type=บทที่2`
        );
  ...
  // ฟังก์ชันสำหรับ ส่งเอกสาร Chapter02
  const handleChapter02Upload = async () => {
    const docType = 'Chapter02';
    try {
      // ประกอบร่างชื่อใหม่
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API}api/project-mgt/getallonedocumenttypeproject?project_id=${projectID}&document_type=บทที่2`
        );
  ...
  const handlePreview = async (FileName) => {
    try {
      const previewResponse = await fetch('/api/download', {
        ...
        body: JSON.stringify({ fileName: FileName, docType: 'Chapter02' }),
        responseType: 'blob',
      });
    ...
    <div>
      <Box style={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
        <Card style={{ width: '60%', borderRadius: 15 }}>
          ...
          >
            <PostAddIcon style={{ marginRight: '0.2rem', height: '5vh' }} /> อัปโหลดเอกสาร Chapter 2
          </Typography>
            ...
            >
              ...
              <Grid justifyContent='center'>
                <WhiteBlackButton
                  ...
                  onClick={() => {
                    handleChapter02Upload();
                    setRefreshFlag((prevFlag) => !prevFlag); // เรียกใช้ useEffect ใน Chapter02Record
                  }}
                >
                  ส่ง
                </WhiteBlackButton>
...
export default Chapter02_upload;
