...
const Chapter05_upload = ({ activeIndex, setActiveIndex, crossdataProject }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setDocumentName('Chapter05_' + response.data.PreprojectData[0].preproject_name_th);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  // ฟังก์ชันสำหรับ ส่งเอกสาร Chapter05
  const handleChapter05Upload = async () => {
    const docType = 'Chapter05';
    try {
      // ประกอบร่างชื่อใหม่
  ...
  const handlePreview = async (FileName) => {
    try {
      const previewResponse = await fetch('/api/download', {
        ...
        body: JSON.stringify({ fileName: FileName, docType: 'Chapter05' }),
        responseType: 'blob',
      });
    ...
    <div>
      <Box style={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
        <Card style={{ width: '60%', borderRadius: 15 }}>
          ...
          >
            <PostAddIcon style={{ marginRight: '0.2rem', height: '5vh' }} /> อัปโหลดเอกสาร Chapter 5
          </Typography>
            ...
            >
              ...
              <Grid justifyContent='center'>
                <WhiteBlackButton
                  ...
                  onClick={() => {
                    handleChapter05Upload();
                    setRefreshFlag((prevFlag) => !prevFlag); // เรียกใช้ useEffect ใน Chapter05Record
                  }}
                >
                  ส่ง
                </WhiteBlackButton>
...
export default Chapter05_upload;
